# -*- coding: utf-8 -*-
# ruff: noqa: F405, F841, E711, E712, F632
#  ↑ 本文件由生成器产出、**策略主体与聚宽原版逐行一致**，故上述告警刻意保留：
#    F405  `from joinquant import *` 是聚宽平台的全局 API（原版 from jqdata import *）
#    F841  原版里有为可读性保留的中间变量，删掉就破坏了"逐行一致"
#    E711  聚宽代码用 `cond == None` 做 **pandas 元素级**比较，换成 `is` 会改变语义
#    E712  同理 `xxx == True/False` 是列掩码，不是真值判断
#    F632  原版用 `x is "字面量"`；虽然不严谨，但改它就偏离了"逐行一致"
#
#  ⚠️ 要改这里的策略逻辑，请改**生成器或聚宽原文件**再重新生成 ——
#     直接编辑本文件会在下次 `python scripts/port_joinquant.py` 时被覆盖。
...
【移植说明】
    ...
    **策略主体与原版逐行一致**，只替换了文件头的 import。
    ...
    原文件：动态持仓 止损线9 单价不超过50-Clone.py
    来源  ：
"""
