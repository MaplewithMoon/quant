# -*- coding: utf-8 -*-
"""基于 frozen 重建清洗层 db/daily
- 读取 frozen/daily_raw（tushare 不复权原始价，可信源）
- 清洗: 删幽灵K线(vol=0/NaN)、价格<=0、OHLC违规、三角违规
- 输出 db/daily（按年/代码分区）
- frozen 层只读，不修改
...
def main():
    ...
    files = glob.glob(f"{SRC}/year=*/*.parquet")
    total_removed = 0
    print(f"重建清洗层: {len(files)} 文件", flush=True)

    for i, f in enumerate(files):
        code = f.split("\\")[-1][:-8]
        year = f.split("\\")[-2].split("=")[1]
        df = pd.read_parquet(f)
        clean, removed = clean_rows(df)
        total_removed += removed
        if clean.empty:
            continue
        part = f"{DST}/year={year}"
        os.makedirs(part, exist_ok=True)
        clean.to_parquet(f"{part}/{code}.parquet", index=False)
    ...
    print("已写入清洗层时间戳 db/daily/_meta.json", flush=True)
...
def verify():
    ...
    r2 = con.execute("""
        WITH t AS (SELECT amount/volume AS vwap, low, high
                   FROM read_parquet('db/daily/year=*/*.parquet') WHERE volume>0)
