...
# 这三只是从行情数据反推出来的（首日涨幅 +188.9% / +122.5% / +235.9%，
# 远超首日 ±44% 的限制，说明当时根本不受该规则约束）。列在这里是为了
# 让 `listing_windows()` 跳过它们 —— 完整名单需要额外数据（见 C2b）。
...
DEFECTS: List[KnownDefect] = [
    ...
    KnownDefect(
        ...
        scope="601399 / 001267 / 601155 三只",
        impact="它们是重组/重新上市，不适用新股首日规则。"
               "实测首日涨幅分别 +188.9% / +122.5% / +235.9%，"
               "被规则误拦 -> 该日**本该能成交却成交不了**（偏悲观）。",
        severity="低",
        needs_data=True,
        mitigation="已知的 3 只已加入 `RELISTED_CODES`，`listing_windows()` 会跳过"
                   "它们（不再误套 ±44%）；完整的「重新上市」名单需要额外数据。",
        codes=["601399", "001267", "601155"],
        doc_ref="三·C2b",
    ),
    KnownDefect(
        key="C4b",
        title="2006–2007 未股改 S 股无涨跌幅限制未建模",
        scope="2006-01-01 ~ 2007-12-31 的主板 S 股",
        impact="残留越界行里 2006/2007 各约 82/81 行。"
               "这些股票当时不设涨跌幅，被套 ±10% 后**误拦**了正常成交（偏悲观）。",
        severity="低",
        needs_data=True,
        mitigation="行数极少且方向保守；根治需要当年的未股改股名单"
                   "（`frozen/stocks` 没有这个字段）。",
        date_range=("2006-01-01", "2007-12-31"),
        doc_ref="三·C4b",
    ),
]
